...
// 10 famílias de cor, 5 tons cada (do mais escuro ao mais claro) = 50 cores
const CORES_FAMILIAS = {
  verde: ['#1b4332', '#2d6a4f', '#40916c', '#74c69d', '#95d5b2'],
  azul: ['#1c3f60', '#2f80ed', '#4dabf7', '#74c0fc', '#a5d8ff'],
  roxo: ['#3a1c71', '#6c63ff', '#9b51e0', '#bb6bd9', '#d0a9f5'],
  vermelho: ['#7a1f2b', '#b23a48', '#e0575f', '#f28b82', '#f7b2ac'],
  laranja: ['#d9480f', '#f2994a', '#f7931e', '#ffab5e', '#ffc48c'],
  amarelo: ['#a67c00', '#f2c94c', '#ffd93d', '#ffe066', '#fff3bf'],
  rosa: ['#a61e63', '#e0578a', '#f783ac', '#ffa8cc', '#ffd6e8'],
  marrom: ['#4e342e', '#795548', '#a1887f', '#c77f5e', '#d7b19d'],
  cinza: ['#212121', '#4f4f4f', '#828282', '#b0b0b0', '#d6d6d6'],
  ciano: ['#086972', '#17a2b8', '#56ccf2', '#76e4f7', '#b2f0fa'],
};
const NOMES_FAMILIAS = {
  verde: 'Verde', azul: 'Azul', roxo: 'Roxo', vermelho: 'Vermelho', laranja: 'Laranja',
  amarelo: 'Amarelo', rosa: 'Rosa', marrom: 'Marrom', cinza: 'Cinza', ciano: 'Ciano',
};
let familiaCorSelecionada = 'verde';
let corSelecionada = CORES_FAMILIAS.verde[1];
...
document.querySelectorAll('.tab').forEach((botao) => {
  botao.addEventListener('click', () => {
    ...
    if (view === 'compras') carregarListaCompras();
  });
});
...
async function carregarCarteiras() {
  const { data, error } = await supabaseClient
    ...
    .order('criado_em');
  if (error) return;
  estado.carteiras = data || [];
  preencherSelectCarteira();
  carregarListaCarteiras();
}

function preencherSelectCarteira() {
  const select = document.getElementById('carteiraLancamento');
  const valorAtual = select.value;
  select.innerHTML = '<option value="">Não informar</option>' + estado.carteiras.map((c) => `<option value="${c.id}">${c.icone} ${c.nome}</option>`).join('');
  select.value = valorAtual;
}

document.getElementById('formCarteira').addEventListener('submit', async (e) => {
  e.preventDefault();
  const nome = document.getElementById('nomeCarteira').value.trim();
  if (!nome) return;

  await supabaseClient.from('carteiras').insert({ grupo_id: estado.grupoId, nome, icone: '👛' });
  e.target.reset();
  carregarCarteiras();
});

function carregarListaCarteiras() {
  const container = document.getElementById('listaCarteiras');
  container.innerHTML = estado.carteiras.length
    ? estado.carteiras.map((c) => `
        <div class="chip">
          <span>${c.icone} ${c.nome}</span>
          <button type="button" class="chip-remover" onclick="excluirCarteira('${c.id}')">✕</button>
        </div>`).join('')
    : '<div class="vazio">Nenhuma carteira ainda.</div>';
}

async function excluirCarteira(id) {
  await supabaseClient.from('carteiras').delete().eq('id', id);
  carregarCarteiras();
}
...
async function carregarPainel() {
  ...
  await carregarSugestaoDiaria();
  await carregarSaldoPorCarteira(inicio, fim);
}

// Sugestão de quanto dá pra gastar por dia até o fim do mês, já
// reservando o que precisa ficar guardado pras metas de economia no
// ritmo combinado (mesmo cálculo do "quanto posso gastar por dia" no bot).
async function carregarSugestaoDiaria() {
  const card = document.getElementById('cardSugestaoDiaria');
  ...
  const metasAtivas = (metas || []).filter((m) => Number(m.valor_guardado) < Number(m.valor_alvo));
  let reserva = 0;
  const nomesMetas = [];
  metasAtivas.forEach((m) => {
    const ritmo = calcularRitmoMetaEconomia(m);
    if (ritmo) {
      reserva += ritmo.sugestaoMensal;
      nomesMetas.push(m.nome);
    }
  });

  const disponivel = Math.max(saldo - reserva, 0);
  const hoje = new Date();
  const ultimoDia = new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0).getDate();
  const diasRestantes = Math.max(ultimoDia - hoje.getDate() + 1, 1);
  const sugestaoDiaria = disponivel / diasRestantes;

  card.hidden = false;
  document.getElementById('valorSugestaoDiaria').textContent = formatarReais(sugestaoDiaria);

  const textoEl = document.getElementById('textoSugestaoDiaria');
  if (nomesMetas.length > 0) {
    const lista = nomesMetas.length === 1 ? nomesMetas[0] : `${nomesMetas.slice(0, -1).join(', ')} e ${nomesMetas[nomesMetas.length - 1]}`;
    textoEl.textContent = `Pra sobrar o suficiente pra guardar rumo à${nomesMetas.length > 1 ? 's metas' : ' meta'} ${lista}, sem atrasar.`;
  } else {
    textoEl.textContent = 'Com base no saldo atual do mês.';
  }
}

// Saldo (receitas - despesas) de cada carteira, dentro do período filtrado.
async function carregarSaldoPorCarteira(inicio, fim) {
  const container = document.getElementById('listaSaldoCarteiras');
  if (estado.carteiras.length === 0) {
    container.innerHTML = '<div class="vazio">Nenhuma carteira cadastrada.</div>';
    return;
  }

  const { data: transacoes } = await supabaseClient
    .from('transacoes')
    .select('valor, tipo, carteira_id')
    .eq('grupo_id', estado.grupoId)
    .gte('data', inicio)
    .lte('data', fim);

  container.innerHTML = estado.carteiras.map((c) => {
    const doCarteira = (transacoes || []).filter((t) => t.carteira_id === c.id);
    const receitas = doCarteira.filter((t) => t.tipo === 'receita').reduce((s, t) => s + Number(t.valor), 0);
    const despesas = doCarteira.filter((t) => t.tipo === 'despesa').reduce((s, t) => s + Number(t.valor), 0);
    const saldo = receitas - despesas;
    return `
      <div class="item">
        <div class="item-principal">
          <div class="item-titulo">${c.icone} ${c.nome}</div>
        </div>
        <div class="item-valor ${saldo >= 0 ? 'pos' : 'neg'}">${formatarReais(saldo)}</div>
      </div>`;
  }).join('');
}
...
// ------------------------------------------------------------
// Calcula quanto seria preciso guardar por mês pra bater a meta no
// prazo combinado, e se está atrasado ou em dia com esse ritmo.
// Sem data_meta definida, retorna null (só mostra o progresso normal).
function calcularRitmoMetaEconomia(meta) {
  if (!meta.data_meta) return null;

  const criado = new Date(meta.criado_em);
  const alvo = new Date(`${meta.data_meta}T00:00:00`);
  const hoje = new Date();

  const mesesEntre = (a, b) => Math.max((b.getFullYear() - a.getFullYear()) * 12 + (b.getMonth() - a.getMonth()), 0);

  const mesesTotal = Math.max(mesesEntre(criado, alvo), 1);
  const sugestaoMensal = Number(meta.valor_alvo) / mesesTotal;
  const mesesPassados = Math.min(mesesEntre(criado, hoje), mesesTotal);
  const esperado = sugestaoMensal * mesesPassados;
  const guardado = Number(meta.valor_guardado);

  return {
    sugestaoMensal,
    esperado,
    atrasado: guardado < esperado - 0.01,
    diferenca: Math.abs(esperado - guardado),
  };
}
...
function renderMetaEconomiaItem(m, comAcoes) {
  ...
  const acoes = comAcoes
    ? `<div class="item-acoes">
         ...
         <button class="botao-icone" onclick="editarMetaEconomiaMiniapp('${m.id}')" title="Editar">✎</button>
  ...
  let statusRitmo = '';
  if (ritmo && !atingida) {
    statusRitmo = ritmo.atrasado
      ? `<div class="status-ritmo atrasado">⏰ Atrasado — sugestão: ${formatarReais(ritmo.sugestaoMensal)}/mês</div>`
      : `<div class="status-ritmo no-prazo">✅ No prazo — sugestão: ${formatarReais(ritmo.sugestaoMensal)}/mês</div>`;
  }
  ...
  return `
    <div class="item meta-item">
      ...
      <div class="barra"><div class="barra-preenchida ${ritmo && ritmo.atrasado && !atingida ? 'estourou' : ''}" style="width:${percentual}%"></div></div>
      ${statusRitmo}
      ${acoes}
    </div>`;
}
...
let metaEconomiaEditandoId = null;
...
document.getElementById('formMetaEconomia').addEventListener('submit', async (e) => {
  ...
  const dataMeta = document.getElementById('dataMetaEconomia').value || null;

  if (metaEconomiaEditandoId) {
    const { data: atual } = await supabaseClient.from('metas_economia').select('valor_guardado').eq('id', metaEconomiaEditandoId).single();
    const atualizacao = { nome, valor_alvo: valorAlvo, data_meta: dataMeta };
    if (atual && valorAlvo > Number(atual.valor_guardado)) atualizacao.celebrada = false;
    await supabaseClient.from('metas_economia').update(atualizacao).eq('id', metaEconomiaEditandoId);
  } else {
    await supabaseClient.from('metas_economia').insert({ grupo_id: estado.grupoId, nome, valor_alvo: valorAlvo, valor_guardado: 0, data_meta: dataMeta });
  }

  encerrarEdicaoMetaEconomia();
  carregarMetaEconomiaLista();
});

document.getElementById('botaoCancelarEdicaoMetaEconomia').addEventListener('click', () => {
  encerrarEdicaoMetaEconomia();
});

function encerrarEdicaoMetaEconomia() {
  metaEconomiaEditandoId = null;
  document.getElementById('formMetaEconomia').reset();
  document.getElementById('botaoSubmitMetaEconomia').textContent = 'Criar meta';
  document.getElementById('botaoCancelarEdicaoMetaEconomia').hidden = true;
}

function editarMetaEconomiaMiniapp(id) {
  const meta = ultimasMetasEconomiaCarregadas.find((m) => m.id === id);
  if (!meta) return;

  metaEconomiaEditandoId = id;
  document.getElementById('nomeMetaEconomia').value = meta.nome;
  document.getElementById('valorMetaEconomia').value = meta.valor_alvo;
  document.getElementById('dataMetaEconomia').value = meta.data_meta || '';
  document.getElementById('botaoSubmitMetaEconomia').textContent = 'Salvar alterações';
  document.getElementById('botaoCancelarEdicaoMetaEconomia').hidden = false;
  document.getElementById('formMetaEconomia').scrollIntoView({ behavior: 'smooth' });
}

let ultimasMetasEconomiaCarregadas = [];
...
async function carregarMetaEconomiaLista() {
  ...
  ultimasMetasEconomiaCarregadas = metas || [];

  const container = document.getElementById('listaMetaEconomia');
  container.innerHTML = ultimasMetasEconomiaCarregadas.length
    ? ultimasMetasEconomiaCarregadas.map((m) => renderMetaEconomiaItem(m, true)).join('')
...
async function pedirContribuicao(metaId, nomeMeta) {
  ...
  const { data: meta } = await supabaseClient.from('metas_economia').select('valor_guardado, valor_alvo, celebrada').eq('id', metaId).single();
  const novoValor = Number(meta.valor_guardado) + valor;
  const atingiuAgora = novoValor >= Number(meta.valor_alvo) && !meta.celebrada;

  const atualizacao = { valor_guardado: novoValor };
  if (atingiuAgora) atualizacao.celebrada = true;
  await supabaseClient.from('metas_economia').update(atualizacao).eq('id', metaId);

  if (tg) tg.HapticFeedback && tg.HapticFeedback.notificationOccurred('success');
  carregarMetaEconomiaLista();
  carregarMetasEconomiaPainel();

  if (atingiuAgora) {
    notificarMetaAtingida(nomeMeta);
  }
}

// Avisa os outros membros do grupo (via bot) que a meta foi batida.
async function notificarMetaAtingida(nomeMeta) {
  const apiBaseUrl = window.API_CONFIG && window.API_CONFIG.baseUrl;
  const apiConfigurada = apiBaseUrl && !apiBaseUrl.includes('seu-bot');
  const initData = tg && tg.initData;
  if (!apiConfigurada || !initData) return;
  ...
  try {
    await fetch(`${apiBaseUrl}/notificar-meta-atingida`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ initData, nomeMeta }),
    });
  } catch (err) {
    console.warn('Não foi possível avisar os outros membros sobre a meta batida.', err);
  }
}

async function excluirMetaEconomia(id) {
  await supabaseClient.from('metas_economia').delete().eq('id', id);
  if (metaEconomiaEditandoId === id) encerrarEdicaoMetaEconomia();
  carregarMetaEconomiaLista();
}
...
async function abrirExportacaoAnual() {
  const { inicio } = calcularIntervaloFiltro();
  ...
  if (!apiConfigurada || !initData) {
    if (tg) tg.showAlert ? tg.showAlert('Configure a API do bot pra exportar o relatório anual.') : alert('Configure a API do bot pra exportar o relatório anual.');
    return;
  }
  ...
  try {
    ...
    console.warn('Não foi possível gerar o link de exportação anual.', err);
    if (tg) tg.showAlert ? tg.showAlert('Não foi possível gerar o relatório agora. Tente de novo.') : alert('Não foi possível gerar o relatório agora.');
    return;
  }

  const url = `${apiBaseUrl}/exportar/pdf-anual?token=${encodeURIComponent(token)}&ano=${ano}`;
  if (tg && tg.openLink) {
    tg.openLink(url);
  } else {
    window.open(url, '_blank');
...
document.getElementById('botaoExportarPdfAnual').addEventListener('click', () => abrirExportacaoAnual());
...
async function desenharGraficoAnel(receitas, despesas) {
  ...
  document.getElementById('legendaAnel').innerHTML = `
    <span><i class="bolinha" style="background:#2d6a4f"></i> Sobra ${formatarReais(dados[0])}</span>
    <span><i class="bolinha" style="background:#2f80ed"></i> Economia ${formatarReais(dados[1])}</span>
    <span><i class="bolinha" style="background:#b23a48"></i> Gastos ${formatarReais(dados[2])}</span>`;
}
...
function desenharGraficoCategoria(transacoes) {
  ...
  // Ordenado do maior gasto pro menor — fica mais fácil de ler assim
  const categoriasOrdenadas = Object.entries(porCategoria)
    .map(([nome, dados]) => ({ nome, ...dados }))
    .sort((a, b) => b.total - a.total);

  const labels = categoriasOrdenadas.map((c) => c.nome);
  const valores = categoriasOrdenadas.map((c) => c.total);
  const cores = categoriasOrdenadas.map((c) => c.cor);
  ...
  const legenda = document.getElementById('legendaGraficoCategoria');
  ...
  graficoCategoria = new Chart(ctx, {
    ...
    options: {
      plugins: {
        legend: { display: false }, // usamos a legenda em HTML abaixo, com valor e %
        tooltip: {
          callbacks: {
            label: (ctx) => {
              const percentual = ((ctx.raw / totalGeral) * 100).toFixed(0);
              return ` ${formatarReais(ctx.raw)} (${percentual}%)`;
            },
          },
        },
      },
  ...
  legenda.innerHTML = categoriasOrdenadas
    .map((c) => {
      const percentual = ((c.total / totalGeral) * 100).toFixed(0);
      return `
        <div class="legenda-item">
          <span class="legenda-cor" style="background:${c.cor}"></span>
          <span class="legenda-nome">${c.nome}</span>
          <span class="legenda-percentual">${percentual}%</span>
          <span class="legenda-valor">${formatarReais(c.total)}</span>
        </div>`;
    })
    .join('');
}
...
async function carregarMetasResumo(inicio, fim) {
  ...
  container.innerHTML = metas.length
    ...
    : '<div class="vazio">Nenhum orçamento definido ainda. Configure na aba Metas.</div>';
}
...
document.getElementById('formLancamento').addEventListener('submit', async (e) => {
  ...
  const carteiraId = document.getElementById('carteiraLancamento').value || null;
  const formaPagamento = document.getElementById('formaPagamentoLancamento').value || null;
...
const NOMES_FORMA_PAGAMENTO_MINIAPP = {
  dinheiro: '💵 Dinheiro',
  pix: '📲 Pix',
  cartao_credito: '💳 Créd.',
  cartao_debito: '💳 Déb.',
...
function renderItemTransacao(t, modoEdicao) {
  const responsavel = t.usuarios ? t.usuarios.nome : '🔁 Automático';
  const carteiraNome = t.carteiras ? t.carteiras.icone + ' ' + t.carteiras.nome : null;
  const formaTexto = t.forma_pagamento ? NOMES_FORMA_PAGAMENTO_MINIAPP[t.forma_pagamento] : null;
  const detalhesExtra = [carteiraNome, formaTexto].filter(Boolean).join(' · ');
    ...
    return `
      <div class="item item-transacao" id="item-${t.id}">
        ...
        <div class="item-rodape">
          <span>${formatarData(t.data)} · ${responsavel}${detalhesExtra ? ' · ' + detalhesExtra : ''}</span>
          <div class="item-acoes">
  ...
  const opcoesCategoria = categoriasDoTipo
    ...
    .join('');
  const opcoesCarteira = '<option value="">Sem carteira</option>' + estado.carteiras
    .map((c) => `<option value="${c.id}" ${c.id === t.carteira_id ? 'selected' : ''}>${c.icone} ${c.nome}</option>`)
    .join('');
  const opcoesForma = ['<option value="">Sem forma</option>']
    .concat(Object.entries(NOMES_FORMA_PAGAMENTO_MINIAPP).map(([chave, texto]) => `<option value="${chave}" ${chave === t.forma_pagamento ? 'selected' : ''}>${texto}</option>`))
    .join('');
  ...
  return `
    <div class="item item-transacao" id="item-${t.id}">
      ...
      <div class="item-linha-topo" style="margin-top:6px">
        <select id="editCarteira-${t.id}" class="select-inline">${opcoesCarteira}</select>
        <select id="editForma-${t.id}" class="select-inline">${opcoesForma}</select>
      </div>
      <div class="item-rodape">
...
async function carregarTransacoes() {
  ...
  const { data: transacoes } = await supabaseClient
    .from('transacoes')
    .select('*, categorias(nome, icone), usuarios(nome), carteiras(nome, icone)')
    .eq('grupo_id', estado.grupoId)
    .gte('data', inicio)
    .lte('data', fim)
...
async function salvarEdicaoTransacao(id) {
  ...
  const carteiraId = document.getElementById(`editCarteira-${id}`).value || null;
  const formaPagamento = document.getElementById(`editForma-${id}`).value || null;
  ...
  await supabaseClient.from('transacoes').update({ categoria_id: categoriaId, valor, descricao, carteira_id: carteiraId, forma_pagamento: formaPagamento }).eq('id', id);
  if (tg) tg.HapticFeedback && tg.HapticFeedback.notificationOccurred('success');
  carregarTransacoes();
}
...
function renderSeletorCorFamilias() {
  const container = document.getElementById('seletorCorFamilias');
  container.innerHTML = Object.entries(CORES_FAMILIAS).map(
    ([chave, tons]) => `<button type="button" class="cor-opcao familia-opcao${chave === familiaCorSelecionada ? ' selecionado' : ''}" data-familia="${chave}" style="background:${tons[1]}" title="${NOMES_FAMILIAS[chave]}"></button>`
  ).join('');
}

function renderSeletorCorTons(familia) {
  const container = document.getElementById('seletorCorTons');
  container.innerHTML = CORES_FAMILIAS[familia].map(
    (cor) => `<button type="button" class="cor-opcao${cor === corSelecionada ? ' selecionado' : ''}" data-cor="${cor}" style="background:${cor}"></button>`
  ).join('');
}

renderSeletorCorFamilias();
renderSeletorCorTons(familiaCorSelecionada);

document.getElementById('seletorCorFamilias').addEventListener('click', (e) => {
  const botao = e.target.closest('.familia-opcao');
  if (!botao) return;
  familiaCorSelecionada = botao.dataset.familia;
  document.querySelectorAll('.familia-opcao').forEach((b) => b.classList.toggle('selecionado', b === botao));
  corSelecionada = CORES_FAMILIAS[familiaCorSelecionada][1];
  document.getElementById('corCategoria').value = corSelecionada;
  renderSeletorCorTons(familiaCorSelecionada);
});

document.getElementById('seletorCorTons').addEventListener('click', (e) => {
  const botao = e.target.closest('.cor-opcao');
  if (!botao) return;
  document.querySelectorAll('#seletorCorTons .cor-opcao').forEach((b) => b.classList.toggle('selecionado', b === botao));
...
document.getElementById('formCategoria').addEventListener('submit', async (e) => {
  ...
  const cor = document.getElementById('corCategoria').value || CORES_FAMILIAS.verde[1];
...
function encerrarEdicaoCategoria() {
  ...
  familiaCorSelecionada = 'verde';
  corSelecionada = CORES_FAMILIAS.verde[1];
  document.getElementById('corCategoria').value = corSelecionada;
  renderSeletorCorFamilias();
  renderSeletorCorTons(familiaCorSelecionada);
...
function editarCategoria(id) {
  ...
  const familiaDaCor = Object.entries(CORES_FAMILIAS).find(([, tons]) => tons.includes(categoria.cor));
  familiaCorSelecionada = familiaDaCor ? familiaDaCor[0] : familiaCorSelecionada;
  renderSeletorCorFamilias();
  renderSeletorCorTons(familiaCorSelecionada);
...
// LISTA DE COMPRAS
// ------------------------------------------------------------
document.getElementById('formItemCompra').addEventListener('submit', async (e) => {
  e.preventDefault();
  const item = document.getElementById('nomeItemCompra').value.trim();
  if (!item) return;

  await supabaseClient.from('lista_compras').insert({ grupo_id: estado.grupoId, item, usuario_id: estado.usuario.id });
...
document.getElementById('botaoLimparComprados').addEventListener('click', async () => {
  await supabaseClient.from('lista_compras').delete().eq('grupo_id', estado.grupoId).eq('comprado', true);
  carregarListaCompras();
});
...
async function carregarListaCompras() {
  ...
  container.innerHTML = (itens || []).length
    ? itens.map((i) => `
        <div class="item item-compra">
          <button type="button" class="item-compra-check ${i.comprado ? 'marcado' : ''}" onclick="alternarItemCompra('${i.id}', ${!i.comprado})">${i.comprado ? '✓' : ''}</button>
          <span class="item-compra-nome ${i.comprado ? 'riscado' : ''}">${i.item}</span>
          <button class="botao-icone" onclick="excluirItemCompraMiniapp('${i.id}')">✕</button>
        </div>`).join('')
    : '<div class="vazio">Lista de compras vazia.</div>';
}

async function alternarItemCompra(id, comprado) {
  await supabaseClient.from('lista_compras').update({ comprado }).eq('id', id);
  if (tg) tg.HapticFeedback && tg.HapticFeedback.notificationOccurred('success');
  carregarListaCompras();
}

async function excluirItemCompraMiniapp(id) {
  await supabaseClient.from('lista_compras').delete().eq('id', id);
  carregarListaCompras();
}
...
// ------------------------------------------------------------
document.getElementById('botaoAbrirCalculadora').addEventListener('click', () => {
  document.getElementById('painelCalculadora').hidden = false;
});
document.getElementById('botaoFecharCalculadora').addEventListener('click', () => {
  document.getElementById('painelCalculadora').hidden = true;
});

const FAMILIA_UNIDADE = { g: 'peso', kg: 'peso', ml: 'volume', l: 'volume', un: 'unidade' };
const ROTULO_BASE = { peso: 'g', volume: 'ml', unidade: 'un' };

// Converte pra unidade "base" da família (grama, mililitro, ou unidade)
function converterParaBase(qtd, unidade) {
  if (unidade === 'kg' || unidade === 'l') return qtd * 1000;
  return qtd;
}

function calcularComparacaoPreco() {
  const qtdA = parseFloat(document.getElementById('calcQtdA').value);
  const unidadeA = document.getElementById('calcUnidadeA').value;
  const precoA = parseFloat(document.getElementById('calcPrecoA').value);

  const qtdB = parseFloat(document.getElementById('calcQtdB').value);
  const unidadeB = document.getElementById('calcUnidadeB').value;
  const precoB = parseFloat(document.getElementById('calcPrecoB').value);

  const resultado = document.getElementById('resultadoCalculadora');

  if (!qtdA || !precoA || !qtdB || !precoB) {
    resultado.innerHTML = '';
    return;
  }

  if (FAMILIA_UNIDADE[unidadeA] !== FAMILIA_UNIDADE[unidadeB]) {
    resultado.innerHTML = '<div class="aviso">Escolha unidades da mesma família pra comparar (peso com peso, ou volume com volume).</div>';
    return;
  }

  const baseA = converterParaBase(qtdA, unidadeA);
  const baseB = converterParaBase(qtdB, unidadeB);
  const precoPorBaseA = precoA / baseA; // preço por grama/ml/unidade — usado só pra comparar
  const precoPorBaseB = precoB / baseB;

  // Pra exibir, converte pra uma escala legível (preço por kg/L, ou por unidade)
  const familia = FAMILIA_UNIDADE[unidadeA];
  const escalaExibicao = familia === 'unidade' ? 1 : 1000;
  const rotulo = familia === 'unidade' ? 'un' : familia === 'peso' ? 'kg' : 'L';
  const precoExibidoA = precoPorBaseA * escalaExibicao;
  const precoExibidoB = precoPorBaseB * escalaExibicao;

  const aMaisBarato = precoPorBaseA < precoPorBaseB;
  const diferenca = Math.abs(((precoPorBaseA - precoPorBaseB) / Math.max(precoPorBaseA, precoPorBaseB)) * 100);

  resultado.innerHTML = `
    <div class="linha-resultado"><span>Produto A</span><span class="${aMaisBarato ? 'vencedor' : ''}">${formatarReais(precoExibidoA)} / ${rotulo}</span></div>
    <div class="linha-resultado"><span>Produto B</span><span class="${!aMaisBarato ? 'vencedor' : ''}">${formatarReais(precoExibidoB)} / ${rotulo}</span></div>
    <div class="linha-resultado" style="margin-top:6px;border-top:1px dashed var(--linha);padding-top:8px;">
      <span>${aMaisBarato ? 'Produto A' : 'Produto B'} é mais barato</span>
      <span class="vencedor">${diferenca.toFixed(0)}% menos por ${rotulo}</span>
    </div>`;
}

['calcQtdA', 'calcUnidadeA', 'calcPrecoA', 'calcQtdB', 'calcUnidadeB', 'calcPrecoB'].forEach((id) => {
  document.getElementById(id).addEventListener('input', calcularComparacaoPreco);
  document.getElementById(id).addEventListener('change', calcularComparacaoPreco);
});
